...
Step 2:
    indicate arguments bio_num and serial by command: run bio2markus.py 50,1 
    or simply import this pakckage
...
def xmlGen(file_name,bio_num=50,serial=1):
    ...
    while file_count in range(0,len(file_name)):
        if bio_count==0:
            f=open(''.join(['./markus/',str(xml_count),'.html']),'w',encoding='utf-8')
            f.write(head)
            f.write('\n')
        bio=pd.read_table(file_name[file_count])
        bio=textWash(bio)
        if serial==1:
        ...
        if bio_count==50 or file_count==len(file_name)-1:
            bio_count=0
            f.write(rear)
            f.close()
